...
int iris_parse_lut_cmds(uint32_t flag)
{
	...
	if (pcfg->iris_chip_type == CHIP_IRIS7)
		ret = iris_parse_lut_cmds_i7(flag);
	else if (pcfg->iris_chip_type == CHIP_IRIS7P)
		ret = iris_parse_lut_cmds_i7p(flag);
	else if (pcfg->iris_chip_type == CHIP_IRIS8)
		ret = iris_parse_lut_cmds_i8(flag);
...
int iris_send_lut(u8 lut_type, u8 lut_table_index)
{
	...
	if (pcfg->iris_chip_type == CHIP_IRIS7)
		ret = iris_send_lut_i7(lut_type, lut_table_index);
	else if (pcfg->iris_chip_type == CHIP_IRIS7P)
		ret = iris_send_lut_i7p(lut_type, lut_table_index);
	else if (pcfg->iris_chip_type == CHIP_IRIS8)
		ret = iris_send_lut_i8(lut_type, lut_table_index);
