...
int iris_configure_t(uint32_t display, u32 type, void __user *argp)
{
	...
	if (pcfg->iris_chip_type == CHIP_IRIS7P)
		...
		ret = iris_configure_i8(display, type, value);
	iris_ioctl_unlock();
		...
		uint32_t count, void __user *values)
{
	...
	if (pcfg->iris_chip_type == CHIP_IRIS7P)
		...
		ret = iris_configure_ex_i8(display, type, count, val);
		...
		uint32_t count, void __user *values)
{
	...
	if (pcfg->iris_chip_type == CHIP_IRIS7P)
		...
		ret = iris_configure_get_i8(display, type, count, val);
	if (ret) {
		IRIS_LOGE("get error");
		vfree(val);
		return ret;
...
int iris_sde_connector_set_metadata(uint32_t value)
{
	...
	if (pcfg->iris_chip_type == CHIP_IRIS7P)
		...
		ret = iris_configure_i8(0, IRIS_SET_METADATA, value);
	iris_ioctl_unlock();
...
int iris_dbgfs_adb_type_init(struct dsi_display *display)
{
	...
	if (pcfg->iris_chip_type == CHIP_IRIS7P)
		...
		iris_dbgfs_adb_type_init_i8(display);
